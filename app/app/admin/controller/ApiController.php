...
class ApiController extends AdminCheckLogin
{
    ...
    public function upload()
    {
        ...
        $auth = new  \Qiniu\Auth(Config::get('qiniu.Access_Key'),config::get('qiniu.Secret_Key'));
        $token = $auth->uploadToken(config::get('qiniu.Bucket'));
        $uploadManager  = new UploadManager();
        $fileInfo = $file->getInfo('ext');
        $saveFileName = date("YmdHis").$fileInfo['name'];
        list($ret, $err) = $uploadManager->putFile($token,$saveFileName,$file->getRealPath());
        if ($err !== null) {
            return vae_assign(0,"上传出错");
        } else {
            return vae_assign(1,"success",['key' => config::get('qiniu.Domain') .'/'. $ret['key']]);
        }
    }
